...
type ServerLinkButtonProps = {
  href: string;
  className?: string;
  children: ReactNode;
} & VariantProps<typeof serverLinkVariants>;

export async function ServerLinkButton({
  href,
  className,
  variant,
  size,
  ...
  return (
    <Link
      href={href}
      className={cn(serverLinkVariants({ variant, size }), className)}
    >
      {children}
    </Link>
  );
